...
%type <sequence> decls exprs insts vdecs varls vars
%type <expression> expr attr addr
%type <lvalue> lval
...
vdecl: type tIDENTIFIER                               { $$ = new gr8::var_decl_node(LINE, 0, $2, nullptr, $1); }
     ;
...
vdefl: type tIDENTIFIER '(' tINIT expr ')'            { $$ = new gr8::var_decl_node(LINE, 0, $2, $5, $1); }
     ;
...
fdef : tDEF qual type tFUNC tIDENTIFIER tAS blocf                   { $$ = new gr8::func_def_node(LINE, $2, $5, new cdk::sequence_node(LINE), $7, $3); }
     ...
     | tDEF type tFUNC tIDENTIFIER tAS blocf                        { $$ = new gr8::func_def_node(LINE, 0, $4, new cdk::sequence_node(LINE), $6, $2); }
     | tDEF type tFUNC tIDENTIFIER tON vdecs tAS blocf              { $$ = new gr8::func_def_node(LINE, 0, $4, $6, $8, $2); }
     ...
     | tDEF tVFUNC tIDENTIFIER tAS blocf                            { $$ = new gr8::func_def_node(LINE, 0, $3, new cdk::sequence_node(LINE), $5, new basic_type()); }
     | tDEF tVFUNC tIDENTIFIER tON vdecs tAS blocf                  { $$ = new gr8::func_def_node(LINE, 0, $3, $5, $7, new basic_type()); }
     ;
...
fdecl: qual type tFUNC tIDENTIFIER                                  { $$ = new gr8::func_decl_node(LINE, $1, $4, new cdk::sequence_node(LINE), $2); }
     ...
     | type tFUNC tIDENTIFIER                                       { $$ = new gr8::func_decl_node(LINE, 0, $3, new cdk::sequence_node(LINE), $1); }
     | type tFUNC tIDENTIFIER tUSES vdecs                           { $$ = new gr8::func_decl_node(LINE, 0, $3, $5, $1); }
     ...
     | tVFUNC tIDENTIFIER                                           { $$ = new gr8::func_decl_node(LINE, 0, $2, new cdk::sequence_node(LINE), new basic_type()); }
     | tVFUNC tIDENTIFIER tUSES vdecs                               { $$ = new gr8::func_decl_node(LINE, 0, $2, $4, new basic_type()); }
     ;
...
smals: tINTDECL                             { $$ = new basic_type(4, basic_type::TYPE_INT); }
     | smals tINTDECL                       { $$ = new basic_type(4, basic_type::TYPE_POINTER); $$->_subtype = $1; }
     ;

yuges: tDBLDECL                             { $$ = new basic_type(8, basic_type::TYPE_DOUBLE); }       
     | yuges tDBLDECL                       { $$ = new basic_type(4, basic_type::TYPE_POINTER); $$->_subtype = $1; }
     ;

fakes: tPOINTER                             { $$ = new basic_type(4, basic_type::TYPE_STRING); }
...
cond : tIF expr tTHEN bloc                  { $$ = new gr8::if_node(LINE, $2, $4); }
     ...
     | tIF expr tTHEN bloc elif             { $$ = new gr8::if_else_node(LINE, $2, $4, $5); }
     ;

elif : tELIF expr tTHEN bloc                { $$ = new gr8::if_node(LINE, $2, $4); }
     | tELIF expr tTHEN bloc elif           { $$ = new gr8::if_else_node(LINE, $2, $4, $5); }
...
expr : tINTEGER                             { $$ = new cdk::integer_node(LINE, $1); }
     ...
     | '(' expr ')'                         { $$ = $2; }
     | tUSE exprs tFOR tIDENTIFIER          { $$ = new gr8::func_call_node(LINE, $4, $2, new basic_type());}
...
lval : tIDENTIFIER                          { $$ = new cdk::identifier_node(LINE, $1); }
     | tINDEXNUM expr tINDEXARR addr        { $$ = new gr8::indexing_node(LINE, $4, $2); }
     ;

addr : tIDENTIFIER                          { $$ = new cdk::rvalue_node(LINE, new cdk::identifier_node(LINE, $1)); }
     | addr '+' tINTEGER                    { $$ = new cdk::add_node(LINE, $1, new cdk::integer_node(LINE, $3)); }
     | tINTEGER '+' addr                    { $$ = new cdk::add_node(LINE, new cdk::integer_node(LINE, $1), $3); }
     | addr '-' tINTEGER                    { $$ = new cdk::sub_node(LINE, $1, new cdk::integer_node(LINE, $3)); }
     | tINTEGER '-' addr                    { $$ = new cdk::sub_node(LINE, new cdk::integer_node(LINE, $1), $3); }
     | '(' addr ')'                         { $$ = $2; }
     ;
