...
namespace gr8 {
  ...
  class postfix_writer: public basic_ast_visitor {
    ...
    bool _funcArgs = false;
    int _currentOffset = -4; // accumulator for in-function local var decls
    int _currentArgOffset = 8; // accumulator for local func args
    std::vector<int> _cycleStarts = std::vector<int>();
    std::vector<int> _cycleEnds = std::vector<int>();
